...
コンテキストスイッチは並行性の本質です ― 実行状態（レジスタ・スタック）を保存し、別の状態を復元して、別のスレッドに「切り替わる」。プリエンプティブスケジューリングは、この切替をタイマ割込みで自動化します。Go ランタイムの M:N スケジューラ、OS のプロセス切替、async フレームワークのタスク切替も、みな同じ構造です。本カリキュラム最大の山場です。

## 背景

Day 08 でスレッド（TCB）のデータ構造と READY リストを設計したが、まだ実際の切り替えはできなかった。本日は**コンテキストスイッチ**（レジスタ・スタックの保存/復元）と、それをタイマ割込みで自動化する**プリエンプティブスケジューリング**を一体化して実装し、真のマルチタスク環境を構築する。

## 新しい概念

- **コンテキストスイッチ**: 実行状態（汎用レジスタ・`esp`・`EFLAGS`）を現在のスレッドのスタックへ保存し、次スレッドのスタックから復元して切り替える処理。TCB の `esp` フィールドが「各スレッドのスタック位置」を記憶する。
- **プリエンプティブスケジューリング**: OS がタイマー割り込みを使って強制的にスレッドを切り替える方式。協調的（スレッドが自発的に CPU を明け渡す）とは対照的で、リアルタイム応答と公平性を保証する。
- **タイムスライス（quantum）**: 各スレッドに与えられる CPU 時間（例: 10ms × N）。tick カウントで管理し、消費すると切替が起きる。
- **レディキュー（READY リスト）**: 実行可能なスレッドの循環リスト。ラウンドロビンで先頭のスレッドに CPU を渡す。
- **スケジューラの呼び出しタイミング**: タイマ割込みハンドラ（IRQ0）内で量子を消費 → 切替判定 → `schedule()` → `context_switch`。割込みコンテキストから呼ばれるため、レジスタ退避（`pusha`/`popa`）が前提になる。
