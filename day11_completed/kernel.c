// Day 11 完成版 - キーボード入力（IRQ1）＋スリープ統合
#include <stdarg.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
// Forward declaration for simple_itoa
...
static void threadA(void) {
    ...
    // Initial delay to stagger thread execution
...
void kmain(void) {
    serial_init();
    serial_puts("KMAIN_START\n");
    serial_puts("D11 kmain begin\n");
    serial_puts("BEFORE_VGA_INIT\n");
    vga_init();
    serial_puts("AFTER_VGA_INIT\n");
    vga_puts("Day 11: Keyboard + Sleep\n");
    serial_puts("AFTER_VGA_PUTS\n");

    init_kernel_context();
    serial_puts("Kernel context initialized, system_ticks=");
    serial_puthex(get_system_ticks());
    serial_puts("\n");
    ...
    serial_puts("About to start multithreading, system_ticks=");
    serial_puthex(get_system_ticks());
    serial_puts("\n");
    schedule();
}
