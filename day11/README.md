...
デバイスドライバの定番パターンを 1 つのデバイスで学びます。割込みハンドラ（生産者）がバッファに書き、アプリ（消費者）が読む、という SPSC（Single Producer Single Consumer）リングバッファは、並行プログラミングで最も古典的かつ安全なパターンです。Linux の evdev 入力サブシステムや、キュー/チャネルを実装するあらゆるシステムも同じ構造です。
