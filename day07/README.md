...
OS が「時間」を扱えるのは、定周期割り込み（tick）があるからです。スケジューラのタイムスライス、システム時刻、タイムアウト、スリープ ― すべて tick から導かれます。Linux の `HZ`、Go ランタイムの sysmon、あらゆるイベントループの心拍源はここにあります。この日で OS に「心臓の鼓動」を与えます。
